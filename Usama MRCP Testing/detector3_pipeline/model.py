# model — leakage-safe training with fold-fitted CSP & Riemann mean
import os, json
import numpy as np
...
from csp_bank_shared import BandTaskCSP  # wrapper: .fit(X, y, task_name), .transform(X)

# ----- Config (match match data_collect) -----
...
def train_models(
    npz_path: str,
    models_dir: str,
    fs_decl: int = 125,   # <- kept for fallback, will be overridden by NPZ if available
    k_folds: int = 5,
):
    os.makedirs(models_dir, exist_ok=True)

    # ---- Load band-limited trials ----
    D = np.load(npz_path, allow_pickle=True)

    # fs from NPZ (sr_ts) 
    fs_from_npz = float(D["sr_ts"]) if "sr_ts" in D.files else float(fs_decl)
    fs_decl = fs_from_npz  # override for the rest of the function
    print(f"Using fs_decl from NPZ sr_ts ≈ {fs_decl:.2f} Hz")
    ...
    for fold, (tr, va) in enumerate(skf.split(np.zeros_like(y4), y4), 1):
        ...
        baseA = Pipeline(baseA_tpl.steps)
        baseB = Pipeline(baseB_tpl.steps)
        baseC = Pipeline(baseC_tpl.steps)

        baseA.fit(X_fbcsp_tr, y4[tr])
        baseB.fit(X_riem_tr,  y4[tr])
        baseC.fit(X_mrcp_tr,  y_active_tr)

        pA = baseA.predict_proba(X_fbcsp_va)
        pB = baseB.predict_proba(X_riem_va)
        pC_full = baseC.predict_proba(X_mrcp_va)
        pC = pC_full[:, 1:2]
        ...
        y_hat_C = np.argmax(pC_full, axis=1)
    ...
    config = {
        ...
        "k_folds": k_folds,
        "fs_from_npz": fs_decl,   # <- saved for reference
        "stack_input_order": [
...
if __name__ == "__main__":
    NPZ = r"C:\Users\rashe\source\repos\MINDUofC\MINDEEG\Usama MRCP Testing\calibration_data\four_class_clench_trials_from_pause.npz" # -_-_-_-_-_-_-_-_CHANGE-_-_-_-_-_-_-_-_-
    MODELS_DIR = r"C:\Users\rashe\source\repos\MINDUofC\MINDEEG\Usama MRCP Testing\calibration_data\models" # -_-_-_-_-_-_-_-_CHANGE-_-_-_-_-_-_-_-_-

    train_models(
        npz_path=NPZ,
        models_dir=MODELS_DIR,
        # fs_decl argument is ignored if NPZ contains 'sr_ts'
        fs_decl=125,
        k_folds=5,
    )
