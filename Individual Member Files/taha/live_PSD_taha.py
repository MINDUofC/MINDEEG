...
class Graph:
        ...
        def _init_timeseries(self):
            ...
            # Initialize lists to store plot and curve objects for each channel and also for the fft
            self.plots = list()
            self.curves = list()
            # self.psd_plots = list() - This is used for when we make separate plots for each PSD curve per channel
            self.psd_curves = list()


            # Expand the window width-wise to fit both time-domain and FFT, just making the window size bigger to accommodate ffts
            self.win.ci.layout.setColumnStretchFactor(0, 1)  # First column (Time-Domain)
            self.win.ci.layout.setColumnStretchFactor(1, 1)  # Second column (FFT)

            # Define a list of colors for the 8 channels
            colors = ['r', 'g', 'b', 'c', 'm', 'y', 'orange', 'purple']

            # Loop through each EEG channel to create a separate plot
            for i in range(len(self.eeg_channels)):  # enumerate(self.eeg_channels):
                ...
                if i == 0:
                    p.setTitle('Raw EEG Signals')   # Set the title for the first plot only of the raw data


                self.plots.append(p)                # Store the plot object in the list
                curve = p.plot(pen=pg.mkPen(colors[i], width=1.5))                    # Create a curve object for plotting data
                self.curves.append(curve)           # Store the curve object in the list

            ### Single Power Spectral Density (PSD) Plot (Right Side)
            p_psd = self.win.addPlot(row=0, col=1, rowspan=8)  # Single plot spanning all rows
            p_psd.setLabel('left', 'PSD (μV²/Hz)')
            p_psd.setLabel('bottom', 'Frequency (Hz)')
            p_psd.showGrid(x=True, y=True)  # Enable grid

            max_freq = self.sampling_rate /2
            p_psd.setXRange(0, max_freq)  # FFT cannot generate frequencies beyond have the samping rate, so this is our upper limit

            p_psd.setLogMode(x=False, y=True)  # Log scale on Y-axis
            p_psd.setTitle('Live PSD (All 8 Channels)')
            p_psd.setYRange(-2, 5)
            self.psd_plot = p_psd  # Store the single PSD plot

            # Create 8 separate curves for PSD, each with a different color
            for i in range(len(self.eeg_channels)):
                curve_psd = p_psd.plot(pen=pg.mkPen(colors[i], width=2))  # Assign color per channel
                self.psd_curves.append(curve_psd)
        ...
        def update(self):
            ...
            # Define frequency bins (assumed same for all channels)
            freqs = np.fft.fftfreq(len(data[0]), d=1 / self.sampling_rate)
            positive_freqs = freqs[:len(freqs) // 2]  # Keep only positive frequencies (the first half of the frequency bins)
            ...
            for count, channel in enumerate(self.eeg_channels):
                ...
                # Now compute the PSD values using fft from numpy
                fft_values = np.fft.fft(data[channel])  # Compute FFT
                power_spectrum = (np.abs(fft_values[:len(freqs) // 2]) ** 2) / len(data[channel])  # Compute Power (PSD)
                # Use a Log Scale for better visualization, linear scaling is not that good
                power_spectrum = np.log1p(power_spectrum)  # log1p(x) = log(1 + x), prevents log(0) issues

                #Apply Smoothing (Rolling Average)
                power_spectrum = uniform_filter1d(power_spectrum, size=8)  # 8-point moving average

                # Update PSD Curve for this channel (Overlay on same plot)
                self.psd_curves[count].setData(positive_freqs, power_spectrum)
