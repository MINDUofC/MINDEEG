...
def draw_between_rounds():
    ...
    message = font.render("Next Round Starting...", True, BLACK)
    round_text = font.render(f"Round {current_round} of {round_mode}", True, BLACK)
    screen.blit(message, (WIDTH//2 - message.get_width()//2, HEIGHT//2 - 40))
    screen.blit(round_text, (WIDTH//2 - round_text.get_width()//2, HEIGHT//2 + 10))

def draw_round_results():
    screen.fill(WHITE)
    round_winner = "Player 1" if winner == "Player 2" else "Player 2"
    result = font.render(f"{round_winner} won the last round!", True, BLACK)
    score_text = font.render(f"Score — P1: {score_p1} | P2: {score_p2}", True, BLACK)
...
def draw_game(rope_x, tug1, tug2):
    ...
    # ✅ Updated labels: Blue = P1 (left), Red = P2 (right)
    label_p1 = small_font.render(f"P1 (N)", True, BLUE)
    label_p2 = small_font.render(f"P2 (B)", True, RED)
...
def reset_game():
    global rope_x, winner, countdown_start, blink_p1, blink_p2, toggle_p1, toggle_p2
    rope_x = WIDTH // 2
    winner = None
